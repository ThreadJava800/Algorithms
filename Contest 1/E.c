...
int main() {
    char word[100000] = "";
    int n = 0;
    INPUT_CHECK(scanf("%s%n", word, &n), 1);

    Stack_t* stack = stkInit();
    
    for (int i = 0; i < n; i++) {
        stkPush(stack, word[i]);
        int tmp = 0;
        while (stack->size >= 2) {
            if (stack->values[stack->size - 1] == ')' && stack->values[stack->size - 2] == '(') {
                stkPop(stack, &tmp);
                stkPop(stack, &tmp);
            }
            else if (stack->values[stack->size - 1] == ']' && stack->values[stack->size - 2] == '[') {
                stkPop(stack, &tmp);
                stkPop(stack, &tmp);
            }
            else if (stack->values[stack->size - 1] == '}' && stack->values[stack->size - 2] == '{') {
